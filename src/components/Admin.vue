...
// GET current value from sheet (via proxy)
...
// POST new value to sheet (via proxy)
// >>> THIS IS WHERE YOUR POST SNIPPET GOES <<<
const saveShots = async () => {
  try {
    ...
    // Keep store in sync so Leaderboard updates immediately
    store.totalShots = totalShots.value;
  } catch (err) {
...
const SHOTS_API_URL = '/.netlify/functions/shots';
